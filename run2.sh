#!/bin/bash
# filepath: /home/skyward/cact/run2.sh
...
for input_file in "${cact_files[@]}"; do
    ...
    output_ll="build/result/${filename}.ll"      # 可选，若还想保留IR
    ...
    # 步骤1: 编译到汇编（直接用compiler生成.s文件）
    echo "  Step 1: Compiling to RISC-V Assembly..." | tee -a "$LOG_FILE"
    if timeout 30s ./build/compiler "$input_file" "$output_ll" "$output_s" 2>> "$LOG_FILE" | tee -a "$tree_file"; then
        if [ -f "$output_s" ] && [ -s "$output_s" ]; then
            echo -e "  ✅ Compiler succeeded" | tee -a "$LOG_FILE"
            
            # 步骤2: 链接生成 RISC-V 可执行文件
            echo "  Step 2: Linking with runtime library..." | tee -a "$LOG_FILE"
            ...
            if timeout 30s $link_cmd 2>&1 | tee -a "$LOG_FILE"; then
                ...
                # 步骤3: 用 QEMU 执行 RISC-V 程序并捕获输出
                echo "  Step 3: Executing program with qemu-riscv64..." | tee -a "$LOG_FILE"
                ...
                exe_exit_code=0
                if [ -f "$input_data_file" ]; then
                    timeout 200s qemu-riscv64 "$output_exe" < "$input_data_file" > "$actual_output_file" 2>&1
                    exe_exit_code=$?
                else
                    timeout 200s qemu-riscv64 "$output_exe" > "$actual_output_file" 2>&1
                    exe_exit_code=$?
                fi
                ...
                # 步骤4: 比较输出
                echo "  Step 4: Comparing output..." | tee -a "$LOG_FILE"
            ...
            echo -e "  ❌ Compiler produced no assembly output" | tee -a "$LOG_FILE"
